from fastapi import APIRouter, Depends
from services import check_role
from models import Roles
from uuid import UUID
from tools import SessionDep
from DTOs import WorkTypeDto
from response import WorkTypeResponse
from services import find_all_work_types, find_work_type_by_id, add_work_type_to_service, update_work_type, delete_work_type, work_type_search

work_type_router = APIRouter(prefix="/work/type")

@work_type_router.get("/all", response_model=list[WorkTypeResponse])
def get_all_work_types(session: SessionDep, id: UUID | None = None):
    return find_all_work_types(id=id, session=session)

@work_type_router.get("")
def get_work_type_by_id(id: UUID, session: SessionDep):
    return find_work_type_by_id(id, session)

@work_type_router.get("/search")
def search_work_type(session: SessionDep, search: str | None = None, service: str | None = None):
    return work_type_search(session=session, search=search, service=service)

@work_type_router.post("/add")
def add_work_type(id: UUID, type_payload: WorkTypeDto, session: SessionDep):
    print(id)
    return add_work_type_to_service(id=id, work_type=type_payload, session=session)

@work_type_router.patch("/update")
def update_type(id: UUID, type_payload: WorkTypeDto, session: SessionDep):
    return update_work_type(id=id, work_type=type_payload, session=session)

@work_type_router.delete("/delete")
def delete_type(id: UUID, session: SessionDep):
    delete_work_type(id, session)